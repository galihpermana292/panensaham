...
function Market() {
	...
	const getQuotes = async () => {
		const data = await marketInfo.getQuotes();
		console.log(data, 'quotes');

		// axios
		// 	.get('http://103.102.177.243:20222/api/v1/quotes', {
		// 		params: { code: '*' },
		// 		headers: { Authorization: 'Bearer 90268887212ac579d5fda22f0ef10381' },
		// 	})
		// 	.then((response) => {
					...
					{indexGraph.map((item, index) => (
						<div className="part-slider">
							...
							<div
								className={
									item.percen > 3.0
										? 'text-left p-2 text-success'
										: 'text-left p-2 text-danger'
								}>
								{item.percen > 3.0 ? '+' + item.percen : '-' + item.percen} %
							</div>
						</div>
					))}
				</Slider>
